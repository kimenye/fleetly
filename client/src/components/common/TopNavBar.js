...
const TopNavBar = ({ currentPage }) => {
  ...
  const getButtonClass = (page) => {
    ...
    return cx({
      "text-white px-3 py-2 rounded-md text-sm font-medium": true,
      "bg-purple-900": isCurrentPage,
      "hover:bg-purple-400": !isCurrentPage
    })
  }
          ...
          <div className="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
            ...
            <div className="hidden sm:block sm:ml-6">
              <div className="flex space-x-4">
                <Link to={ `dashboard` } className={ getButtonClass('dashboard') }>Dashboard</Link>
                <Link to={ `charts` } className={ getButtonClass('charts') }>Charts</Link>
